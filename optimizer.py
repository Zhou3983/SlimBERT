        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if group['t_total'] != -1:
                    ...
                    lr_scheduled = lr * schedule_fct(state['step'] / group['t_total'], group['warmup'])
                else:
                    lr_scheduled = lr
...
def optim4GPU(cfg, model, step_per_epoch):
    """ optimizer for GPU training """
    param_optimizer = list(model.named_parameters())
    no_decay = ['bias', 'gamma', 'beta']

    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if n not in no_decay and 'ib' not in n], 'weight_decay_rate': 0.01,
         'ib_lr': 0.0},
        {'params': [p for n, p in param_optimizer if n in no_decay and 'ib' not in n], 'weight_decay_rate': 0.0,
         'ib_lr': 0.0},
        {'params': [p for n, p in param_optimizer if 'ib' in n], 'weight_decay_rate': 0.0, 'ib_lr': cfg.ib_lr}]

    return BertAdam(optimizer_grouped_parameters,
                    lr=cfg.lr,
                    warmup=cfg.warm_up,
                    t_total=step_per_epoch * cfg.epoch)
